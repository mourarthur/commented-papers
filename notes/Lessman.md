...
This involves:  
1. randomly splitting a data set in half,  
2. using the first and second half for model building and evaluation, respectively,  
3. switching the roles of the two partitions, and  
4. repeating the two-fold validation N times. Compared to using a fixed training and test set, multiple repetitions of two-fold cross-validation give more robust results, especially when working with small data sets. We set N depending on data set size.  
...
1. RL fornece previsões menos acuradas que qualquer dos demais;  
2. HCES-Bag (*Hill-climbing ensemble selection with bootstrap sampling*) fornece previsões mais acuradas que qualquer dos demais; e  
3. os resultados empíricos não permitem concluir se RF e ANN têm performances significantemente diferentes.  
